# Naturalehia

Naturalehia is an MIT-licensed home for open-source projects that help people
observe, understand, and care for the living world.

## Projects

| Project | Mission | Status |
| --- | --- | --- |
| [The Elder Brother of Fauna](projects/the-elder-brother-of-fauna/README.md) | Name tracked animals and estimate the state of individuals and colonies from heterogeneous observations. | Early development |
| [Logic Gates of the Biological Kingdom](projects/logic-gates-of-the-biological-kingdom/README.md) | Build open, evidence-disciplined tools for protein-level Boolean logic research, beginning with an XOR specification. | Stage 1 equilibrium modeling / pre-experimental |
| [Fostering Cellular Agriculture](projects/fostering-cellular-agriculture/README.md) | Build an open financial standard and instrument family for cellular agriculture from research through mass production. | Participation-pool kernel, strict input, CLI, and first facility adapter v0.1 |

Development environments follow the repository's
[one-container-per-active-project convention](CONTAINERS.md).

The repository website starts at [`index.html`](index.html), where the
[projects section](index.html#projects) links to each project’s technical home.

## Licensing

New software and documentation are available under the [MIT License](LICENSE).
Pre-existing visual assets have separate, currently unconfirmed rights; see
[Visual asset licensing](ASSET_LICENSES.md) before reusing them.
