...
struct Options {
    naturalehia::SyntheticConfig source{};
    std::size_t batch_size{256U};
};

void print_help(std::ostream& output) {
    output << "Naturalehia synthetic tracking demo\n\n"
           << "Usage: naturalehia [options]\n\n"
           << "Options:\n"
...
void add_report(naturalehia::BatchReport& total, const naturalehia::BatchReport& batch) noexcept {
...
void print_tracks(std::span<const naturalehia::Track> tracks) {
    ...
    for (const auto& track : tracks) {
        naturalehia::Vec3 position{};
        naturalehia::Vec3 velocity{};
        double population{};
        const bool individual = std::holds_alternative<naturalehia::IndividualState>(track.state);
        if (individual) {
            const auto& state = std::get<naturalehia::IndividualState>(track.state);
            position = naturalehia::position_of(state);
            velocity = naturalehia::velocity_of(state);
        } else {
            const auto& state = std::get<naturalehia::ColonyState>(track.state);
            position = naturalehia::centroid_of(state);
            population = state.estimated_population;
        }

        std::cout << std::left << std::setw(8) << track.id.value << std::setw(16)
                  << track.display_name << std::setw(12) << naturalehia::to_string(track.kind)
                  << std::setw(12) << naturalehia::to_string(track.phase) << std::setw(8)
                  << track.taxon.value << std::setw(8) << track.observation_count;
...
void print_report(const naturalehia::BatchReport& report, std::size_t emitted,
                  std::size_t batches) {
    std::cout << "\nProcessing report\n"
...
int main(int argc, char* argv[]) {
    ...
    try {
        naturalehia::SyntheticSource source(options.source);
        naturalehia::TrackingEngine engine;
        std::vector<naturalehia::Observation> buffer(options.batch_size);
        naturalehia::BatchReport total_report;
        std::size_t emitted{};
        std::size_t batches{};

        for (;;) {
            const naturalehia::ReadResult result = source.read(buffer);
            if (result.count > buffer.size()) {
                throw std::runtime_error("source returned more observations than the buffer holds");
            }
            if (result.status == naturalehia::ReadStatus::error) {
                throw std::runtime_error(result.message.empty() ? "synthetic source failed"
                                                                : result.message);
            }
            if (result.status == naturalehia::ReadStatus::would_block) {
                if (result.count != 0U) {
