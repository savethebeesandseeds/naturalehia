...
\fancyhead[R]{\footnotesize\color{Rust}\textsc{Technical White Paper / Version 1.0}}
...
\begin{titlepage}
  ...
  \vspace{3mm}
  {\LARGE A Transparent Multi-Project Milestone Participation for Financing Research-to-Industry Scale-Up}\par
  \vspace{10mm}
  ...
  \begin{tabularx}{\textwidth}{@{}P{0.27\textwidth}Y@{}}
    ...
    \textbf{Version} & 1.0 / 31 August 2026\\
...
The financial contribution is a source-separated, principal-conserving model that compares an unsupported callable core with two mutually exclusive variants: a fully funded first-loss/priority stack and a failure-contingent partial-credit guarantee. In a ten-claim synthetic experiment with 100 of reference principal, the core has central expected NPV of \(0.661828\) at an 8\% hurdle but adverse expected NPV of \(-18.717674\). Pooling reduces central principal-loss ES95 by 10.05\%, while ES99 diversification benefit is zero. Neither the funded stack nor the 30\% guarantee restores robust investor adequacy under the declared terms. The result is therefore a rejection, not a financing claim. We state the contractual, empirical, institutional and legal evidence required to turn the design into a real asset.
\end{abstract}
...
\begin{enumerate}
  ...
  \item It gives one core construction and two alternative risk-allocation variants without claiming that tranching or guarantees create underlying project value.
...
The synthetic core is barely positive at the central probability mix, yet fails under an admissible adverse mix. Central ES95 diversification exists but disappears at ES99. A funded first-loss layer introduces material prefunding drag, and a 30\% guarantee improves investor NPV only by adding external provider cash; it still leaves a large adverse gap and an unfunded provider budget requirement. The model therefore rejects all three current constructions. This is evidence that the standard can expose a financing gap, not evidence about any live company.
\end{paperbox}
...
The full commitment is subscribed at inception. A funded first-loss claim occupies loss coordinates 0--20 and a priority claim occupies 20--100. Principal cash and unused reserve pay priority first; assigned non-principal cash pays priority up to a lifetime cap and then flows to the first-loss residual. Pool cost is called pro rata. Returned reserve is capital, not profit.

If \(L_s\) is aggregate resolved principal loss in state \(s\), loss allocation is
\begin{align}
L^{\mathrm{FL}}_s &= \min(L_s,20),\\
L^{\mathrm{PR}}_s &= \min\bigl(\max(L_s-20,0),80\bigr).
\end{align}
This variant changes payment timing and priority. It does not change project receipts, gross loss, common-factor dependence or diversification. Full prefunding can reduce call risk but introduces negative carry when capital waits in reserve.

\subsection{Variant 2: failure-contingent partial-credit guarantee}

The alternative guarantee attaches only to the untranched core in version 1. Let \(g\) be the covered share, \(\Gamma\) the provider's legal cash cap and \(L_s\) final resolved loss. With full provider performance,
\begin{align}
G_s &= \min(gL_s,\Gamma),\\
...
NPV at 8\% hurdle & -18.717674 & 0.661828 & 15.440326 \\
...
\bottomrule
\end{tabularx}
\tightnote{Impairment probability is 16\% / 37\% / 60\%. Negative-NPV probability is 28\% / 42\% / 60\%. Each endpoint column except ``central'' is assembled metric by metric and may use a different admissible probability witness.}
\end{table}
...
\begin{axis}[
  ...
  xtick={-30,-20,-10,0,5},
  xlabel={Expected NPV at common 8\% comparison hurdle (\demounit{})},
  xmajorgrids=true,
...
\end{axis}
\end{tikzpicture}
\caption{Expected NPV comparison. The funded stack aggregate uses a common 8\% comparison; its individual classes have separate stated hurdles. Guarantee values are before investor premium.}
\label{fig:npv}
\end{figure}
...
Funded stack, aggregate at 8\% & -8.321750 & -27.985093 & Loss priority becomes real, but full prefunding adds drag and creates no project value. \\
...
The stack subscribes 101 at month zero: 100 of commitment plus 1 of pool cost. Expected prefunding drag at the pool's 8\% hurdle is 8.983578 centrally. At their own stated hurdles, the 0--20 first-loss class has central NPV \(-0.854614\) at 15\%; the 20--100 priority class has central NPV \(-13.926874\) at 8\%. Priority has negative NPV in every declared state. Aggregate stack NPV at the common 8\% hurdle is \(-27.985093/-8.321750/6.715820\) across adverse, central and favorable expected cases.

The result is not an argument against subordination. It shows that subordination must be funded and priced. Loss and exposure reconcile to the unchanged gross pool at every path; rearranging the order of distributions cannot repair insufficient aggregate value.
...
\begin{enumerate}
  ...
  \item \textbf{Explicit risk-bearing support.} A funded first-loss investor or capable guarantor can transfer a defined loss layer if its cost, authority, liquidity and return requirement are transparently financed.
\end{enumerate}
...
\begin{itemize}
  ...
  \item resolved-loss and NPV-shortfall VaR/ES at disclosed levels;
...
Two retained public transaction packages in the repository are useful for question formation but fail the current evidence protocol and do not calibrate this model. No live project is admitted by this paper.
...
\begin{itemize}
  ...
  \item verification status: 68 of 68 declared Debug/Release tests passed in the retained reports;
  \item paper toolchain: \texttt{latexmk} inside the Debian-based \texttt{documents-latex:bookworm} container.
\end{itemize}

The public browser interface is a readable analytical view of frozen synthetic results. A future WebAssembly build may compile the same C++ kernel for interactive use, but it must not be described as parity-verified until Linux-native and WebAssembly structured outputs pass golden-result tests with the same input hashes.
...
The proposed Multi-Project Milestone Participation is a financial standard before it is a product. It defines a real candidate asset as a limited-recourse claim on dated, source-budgeted project cash and recoveries; represents unresolved exposure separately from loss; encodes shared shocks directly; and lets funded subordination or guarantee support be evaluated without confusing transfer with value creation.

The synthetic experiment gives the appropriate first conclusion. Pooling provides some ES95 benefit, but common risk dominates ES99. The central core NPV is slightly positive, yet the adverse permitted NPV is materially negative. Fully funded priority creates drag, and the guarantee adds useful external cash but still cannot satisfy both investor and provider constraints. None of the current constructions is shown financeable.
...
\textbf{Conflicts.} No issuer, investor, guarantor or project is endorsed or admitted.\\
\textbf{Data and code.} Source, configurations, documentation and tests are available in the \href{https://github.com/savethebeesandseeds/naturalehia/tree/main/projects/fostering-cellular-agriculture}{project repository}. The numerical case is synthetic.\\
...
\begin{enumerate}
  \item investor contributions equal project draws plus explicit pool cost, reserve subscription and any separately stated premium;
  ...
  \item tranche loss and continuing exposure sum to the unchanged gross pool amounts;
