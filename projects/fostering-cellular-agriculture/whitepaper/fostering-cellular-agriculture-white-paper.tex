...
\fancyhead[R]{\footnotesize\color{Rust}\textsc{Technical White Paper / Version 1.2}}
...
\begin{titlepage}
  ...
  \begin{tabularx}{\textwidth}{@{}P{0.27\textwidth}Y@{}}
    ...
    \textbf{Version} & 1.2 / 1 September 2026\\
    \textbf{Revision scope} & Capital Mobilization Frontier v0.2 and same-pool issue-price/support sensitivity\\
...
Investors subscribe 100 into the reserve at month zero and contribute 1 as a separate pool-cost call; project uses are funded from the reserve and are not counted again as investor contributions. Expected prefunding drag at the pool's 8\% hurdle is 8.983578 centrally. At their own stated hurdles, the 0--20 first-loss class has central NPV \(-0.854614\) at 15\%; the 20--100 priority class has central NPV \(-13.926874\) at 8\%. Priority has negative NPV in every declared state. Aggregate stack NPV at the common 8\% hurdle is \(-27.985093/-8.321750/6.715820\) across adverse, central and favorable expected cases.
...
Version 0.2 asks a narrower investor question than the construction comparison above: with the ten project paths, event-constrained physical probability set, claim identifiers, lifetime market priority cap \(B=24\), junior hurdle of 15\%, market hurdle of 8\% and zero junior target NPV held fixed, does any declared pair of success-cash participation \(q\) and funded junior issued principal \(A\) satisfy every supplied mandate? The tested grids are
...
\toprule
\textbf{Hurdle \(h\)} & \textbf{Robust ceiling \(P^*\)} & \textbf{Minimum support \(G_{\min}\)} & \textbf{Shortfall to \(G=20\)} & \textbf{Overlap with issuer floor 60} \\
\midrule
...
\begin{itemize}
  ...
  \item verification status: the complete warnings-as-errors C++20 Emscripten 6.0.5 Release build and Node-driven suite pass 72 of 72 declared tests, including the v0.2 asset-to-liability bridge, direct Claim Ledger integration, frontier core/parser/CLI regressions, issue-price browser target and published-WASM byte-parity check;
  \item portable execution: the release evidence reported for version 1.2 is produced by the pinned Emscripten container and Node, including every command-line report wrapper;
