...
Commands:
  ...
  run [ARGS...]   Build and run the synthetic Naturalehia CLI
...
resolve_workspace() {
    ...
    [[ -f "${script_dir}/CMakeLists.txt" ]] ||
        die "setup.sh must remain in the Naturalehia repository root"

    case "$(uname -s)" in
        MINGW* | MSYS* | CYGWIN*)
            require_command cygpath
            HOST_WORKSPACE="$(cygpath --absolute --mixed "${script_dir}")"
            # Prevent MSYS from rewriting Linux container paths passed to docker.exe.
            export MSYS_NO_PATHCONV=1
            ;;
        *)
            HOST_WORKSPACE="${script_dir}"
            ;;
    esac
    [[ "${HOST_WORKSPACE}" != *,* ]] ||
...
verify_existing_configuration() {
    ...
    [[ "${existing_fingerprint}" == "${CONFIG_FINGERPRINT}" ]] ||
        die "container settings changed; run 'bash setup.sh recreate' to apply them"

    [[ "$(docker container inspect --format '{{.Image}}' "${CONTAINER_NAME}")" == "${IMAGE_ID}" ]] ||
        die "managed container image differs from the requested image; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{.Config.User}}' "${CONTAINER_NAME}")" == "${DEV_UID}:${DEV_GID}" ]] ||
        die "managed container user was modified; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{.Config.WorkingDir}}' "${CONTAINER_NAME}")" == "${CONTAINER_WORKSPACE}" ]] ||
        die "managed container work directory was modified; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{.HostConfig.RestartPolicy.Name}}' "${CONTAINER_NAME}")" == "no" ]] ||
        die "managed container restart policy was modified; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{.HostConfig.PidsLimit}}' "${CONTAINER_NAME}")" == "${PIDS_LIMIT}" ]] ||
        die "managed container PID limit was modified; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{.HostConfig.ShmSize}}' "${CONTAINER_NAME}")" == "${SHM_BYTES}" ]] ||
        die "managed container shared-memory limit was modified; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{.HostConfig.Init}}' "${CONTAINER_NAME}")" == "true" ]] ||
        die "managed container init setting was modified; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{json .HostConfig.SecurityOpt}}' "${CONTAINER_NAME}")" == '["no-new-privileges=true"]' ]] ||
        die "managed container security options were modified; run 'bash setup.sh recreate'"
    [[ "$(docker container inspect --format '{{.HostConfig.LogConfig.Type}}' "${CONTAINER_NAME}")" == "local" ]] ||
        die "managed container log driver was modified; run 'bash setup.sh recreate'"
}
...
select_build_directory() {
    ...
    CONTAINER_BUILD_DIR="${CONTAINER_BUILD_ROOT}/${CONFIG_FINGERPRINT}/${toolchain_fingerprint}"
}
...
test_gpu_stack() {
    ...
    as_developer cmake \
        -S "${CONTAINER_WORKSPACE}/cmake/gpu-smoke" \
        ...
        -DNATURALEHIA_CUDA_ARCHITECTURES="${cuda_architectures}" \
        -DNATURALEHIA_TORCH_CUDA_ARCH_LIST="${torch_architectures}" \
...
main() {
    ...
    case "${command}" in
        up)
            ...
            log "enter with: bash setup.sh shell"
            ;;
        shell)
            ensure_ready
        ...
        run)
            ensure_ready
            build_project
            as_developer "${CONTAINER_BUILD_DIR}/naturalehia" "$@"
            ;;
        status)
            show_status
            ;;
