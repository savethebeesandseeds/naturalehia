...
using naturalehia::BatchReport;
using naturalehia::ColonyState;
using naturalehia::ColonySurvey;
using naturalehia::EngineConfig;
using naturalehia::EntityId;
using naturalehia::EntityKind;
using naturalehia::FrameId;
using naturalehia::IndividualDetection;
using naturalehia::IndividualState;
using naturalehia::Observation;
using naturalehia::RenameResult;
using naturalehia::SourceId;
using naturalehia::TaxonId;
using naturalehia::TimePoint;
using naturalehia::TrackingEngine;
using naturalehia::TrackPhase;
using naturalehia::Vec3;
...
const naturalehia::Track& only_track(const TrackingEngine& engine) {
...
void individual_filter_learns_constant_motion() {
    ...
    const Vec3 position = naturalehia::position_of(state);
    const Vec3 velocity = naturalehia::velocity_of(state);
...
void distance_assignment_is_group_wide() {
    ...
    // Sorting all candidate edges instead yields the lower-cost A<-0.0, B<-0.35 assignment.
    const std::array next_frame{
...
void nanosecond_deltas_remain_exact_at_large_timestamps() {
    ...
    REQUIRE(
        ingest_one(engine, individual(105, 2, large_timestamp + 1, {1.0, 0.0, 0.0}, 0.001, 1051))
            .accepted == 1);
    const auto& state = std::get<IndividualState>(only_track(engine).state);
    REQUIRE(naturalehia::velocity_of(state).x > 0.01);
...
std::vector<Observation> collect_synthetic(naturalehia::SyntheticSource& source) {
    ...
    for (;;) {
        ...
        REQUIRE(result.status != naturalehia::ReadStatus::error);
        REQUIRE(result.status != naturalehia::ReadStatus::would_block);
        collected.insert(collected.end(), buffer.begin(),
                         buffer.begin() + static_cast<std::ptrdiff_t>(result.count));
        if (result.status == naturalehia::ReadStatus::end) {
            break;
        }
    }
    return collected;
...
void synthetic_source_is_bounded_and_deterministic() {
    naturalehia::SyntheticConfig config{};
    ...
    naturalehia::SyntheticSource first{config};
    naturalehia::SyntheticSource second_source{config};
...
void synthetic_batches_integrate_and_extremes_fail_cleanly() {
    ...
    naturalehia::SyntheticSource source{config};
    TrackingEngine engine;
    ...
    naturalehia::SyntheticSource all_missed{all_missed_config};
    REQUIRE(all_missed.read(buffer).status == naturalehia::ReadStatus::would_block);
    REQUIRE(all_missed.read(buffer).status == naturalehia::ReadStatus::end);
    ...
    try {
        naturalehia::SyntheticSource invalid_source{unsafe_config};
    } catch (const std::invalid_argument&) {
        source_threw = true;
    }
